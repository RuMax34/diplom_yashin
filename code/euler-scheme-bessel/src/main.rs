...
fn main() {
    ...
    let temperature = 400.0; // temperature in C
    ...
    // See Ga flux calculation below, NEW FORMULA
    let flux_factor = dt * 2.0*d_ga/w.powi(2);
    let zeta = (kr*f_as*tau_as/d_ga).sqrt();
    println!("Bessel function argument is zeta = {}, or zeta*Rd = {}. Bessel ratio for flux = {}.", zeta, zeta*rd_initial, bessel::bessel_ratio(zeta*rd_initial));
    // println!(
    //     "Bessel ratio f(0.11)={}, f(0.55)={}, f(1.17)={}, f(3.8)={}, f(4.99)={}, f(5.01)={}.", 
    //     bessel::bessel_ratio(0.11),
    //     bessel::bessel_ratio(0.55),
    //     bessel::bessel_ratio(1.17),
    //     bessel::bessel_ratio(3.8),
    //     bessel::bessel_ratio(4.99),
    //     bessel::bessel_ratio(5.01),
    // );
...
fn update_concentrations(
    ...
    c_ga_next[0] = c_ga[0] + 4.0 * alpha * (c_ga[1] - c_ga[0])
        - omega * c_ga[0] * c_as[0] + flux_factor*flux_eval(0.0, rd, w, zeta);
    ...
    for j in 1..nr {
        ...
        - omega * c_ga[j] * c_as[j] + flux_factor*flux_eval(j_f64*dr, rd, w, zeta);
...
fn flux_eval(x: f64, rd: f64, w: f64, zeta: f64) -> f64 {
    let z = zeta*rd;
    let p = rd/w;
    let y = x/w;
    let bessel = bessel::bessel_ratio(z);
    let erf = erf::erf_approx(p);
    bessel*(-(y-p).powi(2)).exp()/((-p.powi(2)).exp()+SQRT_PI*p*(1.0+erf))
}
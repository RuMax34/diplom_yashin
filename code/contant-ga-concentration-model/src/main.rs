...
fn main() {
    ...
    let rd_end: f64 = w+2.0*dr;

    // Number of snapshots
    let ns: usize = 10;

    // Initialize arrays
    let mut c_ga: Vec<f64> = r.iter().map(|ri| if *ri < rd_initial + w && *ri > rd_initial - w {1.0} else {0.0}).collect();

...
fn main() {
    ...
    let w: f64 = 1.5;            // width of Gaussian flux, nm
    ...
    let mut c_ga: Vec<f64> = wp(&rj, rd_initial, w);
...
fn update_concentrations(
    ...
    // Special condition at the droplet boundary
    let ja = if rd > 2.0*w {((rd-2.0*w)/dr).floor() as usize} else {0};
    let jb = ((rd+2.0*w)/dr).floor() as usize;
    for j in 0..nr {
        let j_f64 = j as f64;
        // gallium atoms - special condition
        if j >= ja && j < jb {
            c_ga_next[j] = c_ga_next[j].max(wp_j(j_f64*dr, rd, w))
        }
    }
...
fn wp_j(x: f64, x0: f64, w: f64) -> f64 {
    let abs_x = (x-x0).abs();
    let temp = if abs_x < 0.5*w {
        1.0 - abs_x * abs_x / w.powi(2)
    } else if abs_x < 2.0*w {
        let diff = 2.0 - abs_x / w;
        (diff * diff) / 3.0
    } else {
        0.0
    };
...
fn wp(x: &[f64], x0: f64, w: f64) -> Vec<f64> {
    x.iter()
        .map(|&xi| {
            let abs_xi = (xi-x0).abs();
            let temp = if abs_xi < 0.5*w {
                1.0 - abs_xi * abs_xi / w.powi(2)
            } else if abs_xi < 2.0*w {
                let diff = 2.0 - abs_xi / w;
                (diff * diff) / 3.0
            } else {
                0.0
            };

...
fn main() {
    ...
    // Ga initial condition - C0 at droplet boundary
    let ja = if rd_initial > w {((rd_initial-w)/dr).floor() as usize} else {0};
    let jb = ((rd_initial+w)/dr).floor() as usize;
...
fn update_concentrations(
    ...
    let ja = if rd > w {((rd-w)/dr).floor() as usize} else {0};
    let jb = ((rd+w)/dr).floor() as usize;
    for j in 0..nr {
        // gallium atoms - special condition
        if j >= ja && j < jb {
            c_ga_next[j] = c_ga_next[j].max(1.0)
        }
    }
